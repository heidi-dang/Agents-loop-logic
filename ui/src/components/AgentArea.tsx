...
// Heidi Agent Area - Simplified for Heidi backend integration
...
export const AgentArea: React.FC<AgentAreaProps> = ({
  messages, onSendMessage, isTyping, onToggleRightSidebar, isRightSidebarOpen, onApproveTool, onRejectTool, onOpenMobileMenu, backendStatus = 'checking'
}) => {
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:gap-4">
            <h2 className="font-black text-sm text-white tracking-tight flex items-center gap-2">
              <Bot size={16} className="text-white opacity-80" /> HEIDI AGENT
            </h2>
            <div className="flex items-center gap-1.5">
               ...
               <span className={`text-[9px] font-black uppercase tracking-[0.2em] ${
                 ...
                 {backendStatus === 'connected' ? 'AUTONOMOUS MODE' : backendStatus === 'disconnected' ? 'OFFLINE' : 'CONNECTING...'}
               </span>
            </div>
          </div>
        </div>

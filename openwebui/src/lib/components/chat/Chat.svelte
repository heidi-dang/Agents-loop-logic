				...
				<Pane defaultSize={50} minSize={30} class="h-full flex relative max-w-full flex-col">
					...
					<div class="flex flex-col flex-auto z-10 w-full @container overflow-auto">
						{#if ($settings?.landingPageMode === 'chat' && !$selectedFolder) || createMessagesList(history, history.currentId).length > 0}
							...
							{#if !autoScroll && createMessagesList(history, history.currentId).length > 0}
								<div class="absolute bottom-24 right-6 z-20">
									<button
										class="flex items-center gap-2 px-4 py-2.5 bg-purple-600 hover:bg-purple-700 text-white rounded-full shadow-lg transition-all duration-200 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-purple-400 focus:ring-offset-2 focus:ring-offset-gray-900"
										on:click={() => {
											autoScroll = true;
											scrollToBottom('smooth');
										}}
